  ...
  const createMedal = (level: number) => {
    ...
    for (let i = 0; i < level; i++) {
      images.push(
        <img
          key={i}
          src="https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2F1.bp.blogspot.com%2F-TpxKKvThVzg%2FUZMs__NN6DI%2FAAAAAAAASMs%2FUzSjOyVbrCA%2Fs800%2Fgold_medal_ribbon2.png&f=1&nofb=1"
          alt="img"
  ...
  const List = props.medalList.map((data) => {
    return (
      <TableRow key={data.meal_id}>
        ...
        <TableCell>{createMedal(data.level)}</TableCell>
      </TableRow>
    );
  });
  return (
