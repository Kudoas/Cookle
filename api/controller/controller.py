...
@api.route('/<user_id>/follow', methods=['GET'])
@jwt_required
...
@api.route('/<user_id>/unfollow', methods=['GET'])
@jwt_required
...
def user_timeline():
    try:
        ...
        for f in flwee_raw:
            ...
            for f_p in f_posts:
                posts.append({'post_id': f_p.post_id, 'user_id': f.followed_id, 'name': f_name,
                              'meal_url': f_p.recipe_url, 'image_url': f_p.image_url, 'create_at': f_p.create_at})
...
def mypage_json():
    try:
        ...
        n_flwee = len(flwee)
        n_badge = User.query.filter(
            User.user_id == user_id).first().total_badges
        n_point = User.query.filter(
            User.user_id == user_id).first().total_points
...
@api.route('/relation', methods=['GET'])
@jwt_required
def relation_json():
    try:
        user_id = get_user_id()
        ...
        for f in flwer_raw:
            ...
            user_name = User.query.filter(
                User.user_id == f.follower_id).first().name
...
@api.route('/<user_id>/relation', methods=['GET'])
# @jwt_required
...
@api.route('/graph_status', methods=['GET'])
@jwt_required
def graph_status_json():
    user_id = get_user_id()
    #user_id = 1
    graph_url = "https://s3-ap-northeast-1.amazonaws.com/rakuten.intern2020/graph/{}".format(
        user_id)
...
def upload():
    try:
        ...
        response = s3.Bucket('rakuten.intern2020').put_object(
            Key="img/{}/{}".format(user_id, file_name), Body=uploaded_file, ContentType="image/jpeg")
        # URL生成
        ...
        def push_graph():
            ...
            with open("point_graph.html", 'rb') as f:
                ...
                s3 = boto3.resource('s3')
                response = s3.Bucket('rakuten.intern2020').put_object(
                    Key="graph/{}".format(user_id), Body=f, ContentType="text/html")
                # URL生成
...
@api.route('/search/meal', methods=['POST'])
# @jwt_required
@cross_origin()
def search_meal():
    try:
...
# stage2-4 ステータスページ
@api.route('/badge-status', methods=['GET'])
@jwt_required
...
@api.route('/<user_id>/badge-status', methods=['GET'])
@jwt_required
...
@api.route('/total-badge-ranking', methods=['GET'])
@jwt_required
...
def total_badge_ranking_json():
    try:
        ...
        users = db_session.query(
            User.user_id, User.name, User.total_badges, User.total_points).all()
...
@api.route('/total-point-ranking', methods=['GET'])
@jwt_required
...
@api.route('/followee-total-badge-ranking', methods=['GET'])
@jwt_required
...
def followee_total_badge_ranking_json():
    try:
        ...
        for user in flwee:
            flwee_users.append(
                (user.user_id, user.name, user.total_badges, user.total_points))
...
@api.route('/followee-total-point-ranking', methods=['GET'])
@jwt_required
...
@api.route('/monthly-point-ranking', methods=['GET'])
@jwt_required
...
def monthly_point_ranking_json():
    try:
        ...
        users = db_session.query(
            User.user_id, User.name, User.total_badges).all()
        user_list = []
        for user in users:
...
@api.route('/followee-monthly-point-ranking', methods=['GET'])
@jwt_required
...
@api.route('/meal-ranking', methods=['GET'])
@jwt_required
...
@api.route('/weekly-meal-ranking', methods=['GET'])
@jwt_required
def weekly_meal_ranking_json():
    try:
        ...
        for hist in hists:
            ...
            d_create_at = datetime.date(year=int(create_at[:4]), month=int(
                create_at[5:7]), day=int(create_at[8:10]))
